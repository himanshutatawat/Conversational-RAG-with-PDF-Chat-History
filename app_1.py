...
def create_vector_embedding():
    if "vectors" not in st.session_state:
        ...
        # Ensure PDFs exist before loading
        pdf_folder = "research_papers"
        if not os.path.exists(pdf_folder) or not os.listdir(pdf_folder):
            st.error(f"No PDFs found in '{pdf_folder}'. Please add research papers.")
            return
...
if user_prompt:
    ...
    if "vectors" not in st.session_state:
        st.error("Please click 'Document Embedding' to create vector embeddings first.")
        st.stop()
